...
public class Command {
	...
	/**
	 * Bemenetröl érkezö utasítások lefuttatása (tesztekhez)
	 */
	...
	private void railMap(ArrayList<String> mrx){//a sínpálya felépítése
		try{
			...
			for(int i = 0; i < h; i++) {
                ...
                for(int j = 0; j < w; j++){
                    RailElement temp = getElement(row[j]); //végigmegyünk egy sor minden elemén és létrehozzuk a megfelelö objektumot
                    if(temp != null) {
                        railMap[i][j] = temp;
                    }
                }
	...
	private RailElement getElement(String string) { //megfelelö elem létrehozása
