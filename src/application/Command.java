...
public class Command {
	...
	/**
	 * Bemenetről érkező utasítások lefuttatása (tesztekhez)
	 */
	public void init(){ //bemenetről érkező utasítások lefuttatása
		try{
			FileInputStream fis;
            ...
            expected = new ArrayList<>(); //teszt: a várt eredmény ide fog betölteni
            trains.clear();
	...
	private void railMap(ArrayList<String> mrx){//a sínpálya felépítése
		try{
			...
			for(int i = 0; i < h; i++) {
                ...
                for(int j = 0; j < w; j++){
                    RailElement temp = getElement(row[j]); //végigmegyünk egy sor minden elemén és létrehozzuk a megfelelő objektumot
                    if(temp != null) {
                        railMap[i][j] = temp;
                    }
                }
            ...
            for(int i = 0; i < h; i++) {
                for(int j = 0; j < w; j++){
                    if(railMap[i][j] != null) {
                        ...
                        for(int n = 0; n < 4; n++)//végigmegy a szomszédokon, ha valahol érvényes sín van összekapcsolja őket
	...
	private RailElement getElement(String string) { //megfelelő elem létrehozása
	    ...
	    try {
                ...
                switch (comm[0].charAt(0)) {
                    ...
                    case 'C'://Kocsi létrehozása, Carriage vagy CoalCar is lehet
                        ...
                        if (comm.length == 3) {//3 paraméter esetén Carriage
                            if(trains.size() > Character.getNumericValue(comm[0].charAt(1))) {
                                if (comm[1].equals("R")) //piros
                                    c = new Carriage(last, Color.red, comm[2].equals("1") ? true : false); //elötte lévő kocsi, szín és utasok beállítása
                                ...
                                if(((Carriage)c).hasPassangers() && ((Carriage)c).searchToken())//ha vannak utasok, és ő az első megkapja a tokent
                                        c.giveToken();
                            }
                        }
                        ...
                        break;
                    case 'T'://alagút esemény esetén
                        if (comm.length == 4) {//4 paraméteres utasítás
                            int h = Integer.parseInt(comm[1]);
                            int w = Integer.parseInt(comm[2]);
                            if(comm[3].equals("C")) {//ha építeni akarunk
                                if (map[h][w] == null) {
                                    RailElement[] neighs = getNeighbours(h, w); //megkeressük a szomszédban lévő elemeket
                                    for(int n = 0; n < 4; n++) {
                                        if (neighs[n] != null) {//csak akkor építhetünk egy helyre alagutat, ha pontosan egy érvényes sínelem van a szomszédok között
                                            boolean b = true;
                                            for (int j = 0; j < 4; j++)
                                                if (j != n)
	...
	/**
	 * A kész pálya formázása, majd az eredmányhez írása(teszteknél kell)
	 */
	public void getRailMap(){//a pályát formázottan megjeleníti
        ...
        for (int i = 0; i < railMap.length; i++) {
            for (int j = 0; j < railMap[i].length; j++) {
                row += getRailInfo(railMap[i][j]);//mindegyik elemről lekérjük az információt
    ...
    /**
     * A vonatok jelenlegi állását elmenti az eredményekbe
     */
    public void getTrainMap(){//hasonló mint a getRailMap
        updateTrainMap();//először frissítjük a térképet
        String row = "";
        result.add("\n/Trainmap:\n");
        for (int i = 0; i < trainMap.length; i++) {
            for (int j = 0; j < trainMap[i].length; j++) {
